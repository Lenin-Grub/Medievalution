...
sf::Vector2f WorldMap::findProvinceCenter(sf::Color provinceColor)
{
	...
	// Получаем указатель на пиксели и размер карты
	...
	for (unsigned int i = 0; i < mapSize.y; ++i) {
		for (unsigned int j = 0; j < mapSize.x; ++j) {
			...
			// Проверяем, что индекс не выходит за пределы массива пикселей
			if (index + 3 < mapSize.x * mapSize.y * 4) {
				...
				// Обработка ошибок выхода индекса за пределы массива
				return sf::Vector2f(-1.f, -1.f);
			}
		}
	}
	...
	if (pixelCount > 0) {
		...
		// Обработка случая, когда провинция отсутствует на карте
		return sf::Vector2f(-1.f, -1.f);
	}
}
