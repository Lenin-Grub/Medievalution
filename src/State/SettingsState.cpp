...
void SettingsState::updateImGui()
{
    ...
    ImGui::Begin(Localization::get().getString(ICON_SETTINGS, "T_settings").c_str(), nullptr, ImGuiWindowFlags_NoCollapse
...
#pragma region Window settings
    ...
    if (ImGui::BeginCombo(Localization::get().getString(ICON_MONITOR, "T_resolution").c_str(), combo_preview_value, 0))
    {
    ...
    //------------------------------------------------------------------------------------
    if (ImGui::Checkbox(Localization::get().getString(ICON_MONITOR, "T_full_window").c_str(), &WindowSettings::getInstance().fullscreen))
        WindowSettings::getInstance().fullscreen;

    if (ImGui::IsItemHovered())
    {
        ImGui::BeginTooltip();
        ImGui::SetTooltip(Localization::get().getString("T_full_window_tooltip").c_str());
        ImGui::EndTooltip();
    }

    ImGui::BeginDisabled();
    ImGui::Checkbox(Localization::get().getString("T_vertical_sync").c_str(), &WindowSettings::getInstance().vertical_sync);
    ImGui::EndDisabled();

    if (ImGui::IsItemHovered())
    {
        ImGui::BeginTooltip();
        ImGui::SetTooltip(Localization::get().getString("T_vertical_sync_tooltip").c_str());
        ImGui::EndTooltip();
    }

    ImGui::InputInt(Localization::get().getString("T_fps_limit").c_str(), &WindowSettings::getInstance().fps_limit, 0, 1);

    if (ImGui::IsItemHovered())
    {
        ImGui::BeginTooltip();
        ImGui::SetTooltip(Localization::get().getString("T_fps_tooltip").c_str());
        ImGui::EndTooltip();
    }
#pragma endregion
...
#pragma region Sounds and Musics
    ...
    ImGui::SliderInt(Localization::get().getString("T_sound_volume").c_str(), &WindowSettings::getInstance().sound_volume, 0, 100, "%d%%");
    //------------------------------------------------------------------------------------
    ImGui::TableNextColumn();
    ImGui::Checkbox(Localization::get().getString("T_sound").c_str(), &play_sound);
    //------------------------------------------------------------------------------------
    ImGui::TableNextRow();
    ImGui::TableNextColumn();
    if (ImGui::SliderInt(Localization::get().getString("T_music_volume").c_str(), &WindowSettings::getInstance().music_volume, 0, 100, "%d%%"))
        state_machine.data.jukebox.setVolume((int)WindowSettings::getInstance().music_volume);
    //------------------------------------------------------------------------------------
    ImGui::TableNextColumn();
     if (ImGui::Checkbox(Localization::get().getString("T_music").c_str(), &play_music))
     {
         if (play_music)
...
#pragma region Camera settings 
    ImGui::Separator();
    ImGui::BeginDisabled();
    ImGui::SliderFloat(Localization::get().getString("T_zoom_speed").c_str(), &WindowSettings::getInstance().zoom_speed, 0, 1, "%.1f");
    ImGui::SliderInt(Localization::get().getString("T_camera_speed").c_str(), &WindowSettings::getInstance().camera_speed, 1, 100, "%d%%");
    ImGui::EndDisabled();
#pragma endregion

#pragma region Apply 
    ImGui::Separator();
    if (ImGui::Button(Localization::get().getString("T_apply").c_str()))
    {
        ImGui::OpenPopup(Localization::get().getString("T_apply").c_str());
    }
#pragma endregion

#pragma region Apply  settings
    ImGui::SetNextWindowPos(center, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
    if (ImGui::BeginPopupModal(Localization::get().getString("T_apply").c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize))
    {
        ImGui::Text(Localization::get().getString("T_changed_game_settings").c_str());
        ImGui::Separator();

        if (ImGui::Button(Localization::get().getString("T_yes").c_str(), ImVec2(120, 0)))
        {
        ...
        ImGui::SetItemDefaultFocus();
        ImGui::SameLine();
        if (ImGui::Button(Localization::get().getString("T_cancel").c_str(), ImVec2(120, 0))) { ImGui::CloseCurrentPopup(); }
        ImGui::EndPopup();
    }
#pragma endregion
...
#pragma region Exit and metrics
    ...
    ImGui::SameLine();
    if (ImGui::Button(Localization::get().getString("T_exit").c_str()))
    {
        state_machine.lastState();
    }
    ImGui::End();
