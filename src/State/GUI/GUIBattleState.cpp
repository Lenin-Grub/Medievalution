...
#pragma region Editor
    ImGui::Begin((ICON::iconToUnicode(ICON_MAP) + " Editor").c_str(), nullptr, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysVerticalScrollbar);

    if (ImGui::CollapsingHeader((std::string(ICON::iconToUnicode(ICON_STACK_FILES)) + " Layers").c_str()))
    {
        ...
        if (ImGui::Button((ICON::iconToUnicode(ICON_ADD_FILES) + " Add").c_str()))
        ...
        if (ImGui::Button((ICON::iconToUnicode(ICON_REMOVE_FILES) + " Remove").c_str()))
        ...
        if (ImGui::BeginTable("LayersTable", 3))
        {
            ...
            for (size_t i = 0; i < layers.size(); ++i)
            {
                ...
                std::string display_layer_name = ICON::iconToUnicode(ICON_MAP).c_str() + (layer->name.empty() ? default_layer_name : layer->name);
                ...
                ImGui::PushID(static_cast<int>(i));
                if (ImGui::Button((ICON::iconToUnicode(ICON_EDIT)).c_str()))
    ...
    //______________________________________
    if (ImGui::CollapsingHeader((ICON::iconToUnicode(ICON_FOUR_QUADS) + " Tiles").c_str()))
    {
...
#pragma  region Animator
    {
        ImGui::Begin((ICON::iconToUnicode(ICON_INSTAGRAM) + " Animation").c_str(), nullptr);
        ...
        if (ImGui::Button((ICON::iconToUnicode(ICON_ADD_FILES) + " Add animation").c_str()))
            m_show_popup = true;
        ...
        if (ImGui::Button((ICON::iconToUnicode(ICON_BEGIN) + "##Begin").c_str()))
        {
            cf = battleState.animator.getFirstFrame();
            battleState.animator.setCurrentFrame(cf);
        }
        ImGui::SameLine();
        if (ImGui::Button((ICON::iconToUnicode(ICON_PREV) + "##Prev").c_str()))
        {
            cf = battleState.animator.getPrevFrame();
            battleState.animator.setCurrentFrame(cf);
        }
        ImGui::SameLine();
        if (ImGui::Button(pl ? (ICON::iconToUnicode(ICON_PLAY)  + "##Play" ).c_str()
                             : (ICON::iconToUnicode(ICON_PAUSE) + "##Pause").c_str()))
        {
            pl = !pl;
        }
        ImGui::SameLine();
        if (ImGui::Button((ICON::iconToUnicode(ICON_NEXT) + "##Next").c_str()))
        ...
        ImGui::SameLine();
        if (ImGui::Button((ICON::iconToUnicode(ICON_END) + "##End").c_str()))
        {
        ...
        if (ImGui::CollapsingHeader("Frames"))
        {
            ...
            for (size_t i = 0; i < frames.size(); ++i)
            {
                ...
                std::string frame_name = ((ICON::iconToUnicode(ICON_EMPTY_FILES) + "Frame").c_str()) + std::to_string(i);
